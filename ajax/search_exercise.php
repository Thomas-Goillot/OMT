...
if(isset($search)){
    $get_exercise = $bdd->prepare('SELECT * FROM EXERCISE WHERE exercise_name LIKE ? ORDER BY exercise_name LIMIT 10');
    $get_exercise->execute(array("$search%"));
    $get_exercise_info = $get_exercise->fetchAll();
}
else{
    $get_exercise = $bdd->prepare('SELECT * FROM EXERCISE ORDER BY exercise_name LIMIT 10');
